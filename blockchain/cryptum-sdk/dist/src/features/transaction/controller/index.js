...
const { Protocol, TRANSFER_METHOD_ABI, TRANSFER_COMMENT_METHOD_ABI, CUSD_CONTRACT_ADDRESS, CEUR_CONTRACT_ADDRESS, } = require('../../../services/blockchain/constants');
const { getTokenAddress, toWei, toHTRUnit } = require('../../../services/blockchain/utils');
const { FeeResponse, TransactionResponse, SignedTransaction, UTXO, TransactionType, SmartContractCallResponse, Input, } = require('../entity');
...
const { buildBscTransferTransaction, buildEthereumTransferTransaction, buildEthereumSmartContractTransaction, buildEthereumSmartContractDeployTransaction, } = require('../../../services/blockchain/ethereum');
...
const { buildCeloSmartContractTransaction, buildCeloTransferTransaction, buildCeloSmartContractDeployTransaction, } = require('../../../services/blockchain/celo');
const { validateCeloTransferTransactionParams, validateBitcoinTransferTransactionParams, validateSignedTransaction, validateSmartContractTransactionParams, validateSmartContractCallParams, validateSmartContractDeployTransactionParams, validateTokenDeployTransactionParams, validateRippleTransferTransactionParams, validateStellarTransferTransactionParams, validateStellarTrustlineTransactionParams, validateRippleTrustlineTransactionParams, validateEthereumTransferTransactionParams, validateHathorTokenTransactionFromUTXO, validateHathorTokenTransactionFromWallet, validateHathorTransferTransactionFromWallet, validateHathorTransferTransactionFromUTXO, } = require('../../../services/validations');
...
class Controller extends Interface {
    ...
    async sendTransaction(transaction) {
        try {
            ...
            const response = await apiRequest(requests.sendTransaction.url, { signedTx, type }, {
                headers,
                params: { protocol },
            });
     ...
     */
    async getFee({ type = null, from = null, amount = null, destination = null, contractAddress = null, contractAbi = null, method = null, params = null, protocol, contractName = null, source = null, feeCurrency = null, tokenType = null, }) {
        try {
            const apiRequest = getApiMethod({
                requests,
                key: 'getFee',
    ...
    async createBitcoinTransferTransaction(input) {
        ...
        if (wallet) {
            ...
            for (let i = 0; i < utxos.length; ++i) {
                ...
                inputs[i] = new Input(Object.assign(Object.assign({}, utxos[i]), { privateKey: wallet.privateKey, hex: tx.hex, blockhash: tx.blockhash }));
            }
        }
        else if (inputs) {
    ...
    async createSmartContractTransaction(input) {
        ...
        if (protocol === Protocol.CELO) {
            ...
            signedTx = await buildEthereumSmartContractTransaction(Object.assign(Object.assign({}, transactionOptions), { protocol }));
        }
        else {
    ...
    async callSmartContractMethod(input) {
        ...
        try {
            ...
            const headers = mountHeaders(this.config.apiKey);
            const response = await apiRequest(requests.callSmartContractMethod.url, { from, contractAddress, contractAbi, method, params }, {
                headers,
                params: { protocol },
            });
    ...
    async _getFeeInfo({ wallet, type, destination, amount, contractAddress, contractAbi, method, params, fee, feeCurrency, protocol, contractName, source, tokenType, }) {
        ...
        if (fee && fee.gas)
            networkFee.gas = fee && fee.gas;
        if (fee && fee.gasPrice)
            networkFee.gasPrice = fee && fee.gasPrice;
    ...
    async createSmartContractDeployTransaction(input) {
        ...
        if (protocol === Protocol.CELO) {
            ...
            signedTx = await buildEthereumSmartContractDeployTransaction(Object.assign(Object.assign({}, transactionOptions), { protocol }));
        }
        else {
            throw new GenericException('Invalid protocol', 'InvalidTypeException');
        }
        ...
        for (let i = 0; i < utxos.length; ++i) {
            if (tokens.includes(utxos[i].token)) {
                ...
                if (utxos[i].token === '00' ||
                    ...
                    inputs.push(Object.assign(Object.assign({}, utxos[i]), { privateKey: wallet.privateKey }));
                }
            }
        }
    ...
    async createHathorTokenTransactionFromWallet(input) {
        ...
        let { wallet, tokenName, tokenSymbol, type, tokenUid, mintAuthorityAddress, meltAuthorityAddress, amount, address, changeAddress, testnet, } = input;
        let inputSum = 0;
    ...
    async createHathorTokenTransactionFromUTXO(input) {
        ...
        let { inputs, tokenName, tokenSymbol, address, changeAddress, mintAuthorityAddress, meltAuthorityAddress, amount, tokenUid, type, testnet, } = input;
        let inputSum = 0;
        const protocol = Protocol.HATHOR;
        for (let i = 0; i < inputs.length; ++i) {
            const tx = await this.getTransactionByHash({ hash: inputs[i].txHash, protocol });
    ...
    async createCardanoTransferTransactionFromWallet(input) {
        try {
            ...
            const metadata = await apiRequest(`${requests.getMetadata.url}?protocol=${protocol}`, Object.assign({}, options.data));
            ...
            const signatures = payload.data.payloads.map((signing_payload) => {
                const { account_identifier: { address }, } = signing_payload;
                return {
                    signing_payload,
                    public_key: {
                    ...
                    hex_bytes: Buffer.from(CardanoWasm.make_vkey_witness(CardanoWasm.TransactionHash.from_bytes(Buffer.from(signing_payload.hex_bytes, 'hex')), privateKey.to_raw_key())
                        .signature()
        ...
        try {
            ...
            await Promise.all(inputs.map(async (i) => {
                ...
                utxo.map((u) => {
                    if (u.txHash === i.txHash && u.index.toString() === i.index) {
                        inputList.push(Object.assign(Object.assign({}, u), { address: i.address }));
                    }
                });
            }));
            ...
            const signatures = payload.data.payloads.map((signing_payload) => {
                ...
                return {
                    ...
                    signature_type: 'ed25519',
                    hex_bytes: Buffer.from(CardanoWasm.make_vkey_witness(CardanoWasm.TransactionHash.from_bytes(Buffer.from(signing_payload.hex_bytes, 'hex')), keyAddressMapper[address].secretKey.to_raw_key())
                        .signature()
                        .to_bytes()).toString('hex'),
                };
            });
