...
hathorLib.transaction.updateTransactionWeightConstants(hathorLib.constants.TX_WEIGHT_CONSTANTS.txMinWeight, hathorLib.constants.TX_WEIGHT_CONSTANTS.txWeightCoefficient, hathorLib.constants.TX_WEIGHT_CONSTANTS.txMinWeightK);
...
module.exports.buildHathorTransferTransaction = async function ({ inputs, outputs, tokens, changeAddress, testnet }) {
    ...
    const tokenAmountsCheck = getInputOutputAmountsCheck({
        inputs,
        outputs: outputs.map((o) => (Object.assign(Object.assign({}, o), { value: toHTRUnit(o.amount).toNumber() }))),
        tokens,
    });
    if (changeAddress) {
...
module.exports.buildHathorTokenTransaction = async function ({ inputs, tokenName, tokenSymbol, address, changeAddress, mintAuthorityAddress, meltAuthorityAddress, amount, inputSum, tokenUid, type, testnet, }) {
    let txData = {
        inputs: inputs.map((input) => ({ tx_id: input.txHash, index: input.index })),
        outputs: []
    };
    const tokenAmount = toHTRUnit(amount).toNumber();
    const _dataToken = type === TransactionType.HATHOR_TOKEN_MELT
        ? txData
