...
const { CUSD_CONTRACT_ADDRESS, CEUR_CONTRACT_ADDRESS, TRANSFER_METHOD_ABI, TRANSFER_COMMENT_METHOD_ABI, CELO_CONTRACT_ADDRESS, } = require('./constants');
const { compileContract } = require('../../services/blockchain/contract');
module.exports.buildCeloTransferTransaction = async function ({ fromPrivateKey, nonce, tokenSymbol, contractAddress, amount, destination, fee, feeCurrency = null, memo = null, testnet = true, }) {
...
module.exports.buildCeloSmartContractTransaction = async ({ fromPrivateKey, nonce, contractAddress, contractAbi, method, params, value, fee, feeCurrency, testnet, }) => {
    const network = testnet ? 'testnet' : 'mainnet';
    const { gas, gasPrice, chainId } = fee;
    const rawTransaction = {
        chainId,
...
module.exports.buildCeloSmartContractDeployTransaction = async ({ fromPrivateKey, contractName, source, nonce, fee, feeCurrency, testnet, config, tokenType, params, }) => {
