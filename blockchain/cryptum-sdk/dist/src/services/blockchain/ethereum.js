...
module.exports.buildEthereumTransferTransaction = async function ({ fromPrivateKey, nonce, tokenSymbol, contractAddress, amount, destination, fee, }) {
...
module.exports.buildBscTransferTransaction = async function ({ fromPrivateKey, nonce, tokenSymbol, contractAddress, amount, destination, fee, testnet, }) {
    const { gas, gasPrice, chainId } = fee;
    const rawTransaction = {
        chainId,
        nonce: Web3.utils.toHex(nonce),
...
module.exports.buildEthereumSmartContractTransaction = async ({ fromPrivateKey, nonce, contractAddress, contractAbi, method, params, value, fee, testnet, protocol, }) => {
    const network = testnet ? 'testnet' : 'mainnet';
    const { gas, gasPrice, chainId } = fee;
    const rawTransaction = {
        chainId,
...
module.exports.buildEthereumSmartContractDeployTransaction = async ({ fromPrivateKey, nonce, contractName, source, fee, testnet, protocol, tokenType, config, params, }) => {
