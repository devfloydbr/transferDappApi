module.exports.buildOperation = (unspents, address, outputs, fee = 200000) => {
    ...
    const inputs = unspents.map((coin, index) => {
        ...
        if (coin.metadata) {
            ...
            const coinTokenBundleItems = coinsWithMa.reduce((tokenBundleList, coinWithMa) => {
    ...
    const outputOp = outputs.map((output) => {
        ...
        if (!output.token) {
            ...
            try {
                ...
                throw new Error('Native token amount must be an integer');
            }
    ...
    const leftoverAdaAmount = BigInt(totalBalance * 1000000)
    ...
    if (tokenBundle.length > 0 && isAdaOnly)
...
module.exports.buildOperationFromInputs = (unspents, outputs) => {
    let tokenBalanceMap = {};
    unspents.map((output) => {
        if (output.metadata) {
            output.metadata[`${output.txHash}:${output.index}`].map((policy) => {
