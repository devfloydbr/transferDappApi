...
const getBitcoinDerivationPath = ({ account = 0, change, address }) => getDerivationPath({ purpose: 44, coin: 0, account, change, address });
const getTestnetDerivationPath = ({ account = 0, change, address }) => getDerivationPath({ purpose: 44, coin: 1, account, change, address });
const getEthereumDerivationPath = ({ account = 0, change, address }) => getDerivationPath({ purpose: 44, coin: 60, account, change, address });
const getCeloDerivationPath = ({ account = 0, change, address }) => getDerivationPath({ purpose: 44, coin: 52752, account, change, address });
const getHathorDerivationPath = ({ account = 0, address }) => getDerivationPath({ purpose: 44, coin: 280, account, address });
const getCardanoDerivationPath = ({ account = 0, address }) => getDerivationPath({ purpose: 1852, coin: 1815, account, address });
const getAvalancheDerivationPath = ({ account = 0, address }) => getDerivationPath({ purpose: 44, coin: 9000, account, address });
/**
 ...
 * @returns
 */
module.exports.deriveBitcoinWalletFromDerivationPath = async (mnemonic, testnet, { account = 0, change = 0, address } = {}) => {
    const network = testnet ? bitcoin.networks.testnet : bitcoin.networks.bitcoin;
...
module.exports.deriveCeloAddressFromXpub = async (xpub, { address = 0 } = {}) => this.deriveEthereumAddressFromXpub(xpub, { address });
module.exports.deriveBscAddressFromXpub = async (xpub, { address = 0 } = {}) => this.deriveEthereumAddressFromXpub(xpub, { address });
/**
...
module.exports.deriveCardanoWalletFromDerivationPath = async (mnemonic, testnet, { account = 0, address = 0 } = {}) => {
    ...
    const baseAddr = CardanoWasm.BaseAddress.new(testnet ? 0 : 1, CardanoWasm.StakeCredential.from_keyhash(utxoPubKey.to_raw_key().hash()), CardanoWasm.StakeCredential.from_keyhash(stakingPubKey.to_raw_key().hash())).to_address().to_bech32();
    return {
...
module.exports.deriveCardanoAddressFromXpub = async (xpub, testnet) => {
    ...
    const baseAddr = CardanoWasm.BaseAddress.new(testnet ? 0 : 1, CardanoWasm.StakeCredential.from_keyhash(utxo.hash()), CardanoWasm.StakeCredential.from_keyhash(stake.hash())).to_address().to_bech32();
    return baseAddr;
};
module.exports.getCardanoAddressFromPrivateKey = (privateKey, testnet = true) => {
    ...
    const stakingPubKey = CardanoWasm.Bip32PrivateKey.from_128_xprv(stakingPrivateKey)
        .to_public();
    const baseAddr = CardanoWasm.BaseAddress.new(testnet ? 0 : 1, CardanoWasm.StakeCredential.from_keyhash(spendingPublicKey.to_raw_key().hash()), CardanoWasm.StakeCredential.from_keyhash(stakingPubKey.to_raw_key().hash())).to_address().to_bech32();
    return baseAddr;
};
/**
...
module.exports.deriveAvalancheAddressFromXpub = async (xpub, { address = 0 } = {}) => this.deriveEthereumAddressFromXpub(xpub, { address });
