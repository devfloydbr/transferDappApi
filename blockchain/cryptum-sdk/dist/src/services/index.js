...
const handleRequestError = (error) => {
    if (error) {
        if (error.response) {
            const message = (error.response.data.error && error.response.data.error.message) || (error.response.data.message) || 'Service unavailable at the moment';
...
const mountHeaders = (apiKeyCryptum) => {
    if (!apiKeyCryptum)
        throw new GenericException('Required API Key', 'ConfigException');
